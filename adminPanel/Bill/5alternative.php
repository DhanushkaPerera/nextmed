<html>

<head>
	<link rel="stylesheet" href="table.css"/>
</head>

<body>
		<div class="back" style="float:right;cursor:pointer;color:#0033cc;width:30px;height:30px;border-radius:100%;text-align:center;line-height:30px;" onclick="goBack()">X</div>
<!DOCTYPE html>
<html>

<head>
	<link rel="stylesheet" href="table.css"/>
</head>

<body>
		<p>We <font color=red>haven't</font> what exactky customer wants. But alternatives are in.</p><div class="back" style="float:right;cursor:pointer;color:#0033cc;width:30px;height:30px;border-radius:100%;text-align:center;line-height:30px;" onclick="goBack()">X</div>
	
	
	<div class="doctortable">

	<table>
	
		<tr>
			<th>Alternative Name</th>
			<th>Dosage per Person</th>
			<th>Health Tips</th>
			<th>Allergic Effect</th>
			<th>Usable Period</th>
			<th>Quantity</th>
			<th>Discount</th>
			<th>Dis.Price</th>
			<th>Price Comparison</th>
			
			
		</tr>
		
		<tr>
			<td>Paracetamol</td>
			
			<td>1 tablet per 32 kg</td>
			<td>Drink more water during day</td>
			<td>Ok</td>
			<td>30 days</td>
			<td><input name="checked" id="checkAgree" value="" type="text" onclick="validatedAll()"/></td>
			<td><input name="checked" id="checkAgree" value="Bill" type="Submit" onclick="validatedAll()"/></td>
		
		</tr>
		
		<tr>
			<td>Paracetamol</td>
			
			<td>1 spoon per 32 kg</td>
			<td>Drink more water during day</td>
			<td>Ok</td>
			<td>15 days</td>
			<td><input name="checked" id="checkAgree" value="" type="text" onclick="validatedAll()"/></td>
			<td><input name="checked" id="checkAgree" value="Bill" type="Submit" onclick="validatedAll()"/></td>
		
		</tr>
		
		<tr>
			<td>Paracetamol</td>
			
			<td>1 spoon per 32 kg</td>
			<td>Drink more water during day</td>
			<td>Ok</td>
			<td>60 days</td>
			<td><input name="checked" id="checkAgree" value="" type="text" onclick="validatedAll()"/></td>
			<td><input name="checked" id="checkAgree" value="Bill" type="Submit" onclick="validatedAll()"/></td>
		
		</tr>
		
		<tr>
			<td>Paracetamol</td>
			
			<td>1 tablet per 32 kg</td>
			<td>Drink more water during day</td>
			<td>Not ok</td>
			<td>15 days</td>
			<td><input name="checked" id="checkAgree" value="" type="text" onclick="validatedAll()"/></td>
			<td><input name="checked" id="checkAgree" value="Bill" type="Submit" onclick="validatedAll()"/></td>
		
		</tr>
		
		<tr>
			<td>Paracetamol</td>
			
			<td>1 spoon per 32 kg</td>
			<td>Drink more water during day</td>
			<td>don't know<br>customer al. con.<br>(not regi cus)</td>
			<td>15 days</td>
			<td><input name="checked" id="checkAgree" value="" type="text" onclick="validatedAll()"/></td>
			<td><input name="checked" id="checkAgree" value="Bill" type="Submit" onclick="validatedAll()"/></td>
		
		</tr>
		
		<tr>
			<td>Paracetamol</td>
			
			<td>1 spoon per 32 kg</td>
			<td>Drink more water during day</td>
			<td>Drug allergic <br>effect(general)</td>
			<td>15 days</td>
			<td><input name="checked" id="checkAgree" value="" type="text" onclick="validatedAll()"/></td>
			<td><input name="checked" id="checkAgree" value="Bill" type="Submit" onclick="validatedAll()"/></td>
		
		</tr>
		
		
		
		
		
	</table>
	</div>
	
</body>

</html>

<script>
function goBack() {
    window.history.back();
}
</script>	</body>
<html>